...
class MainApp(Window):
    ...
    def __init__(self):
        super().__init__("pgAdmin4", 350, 200)
        self.attributes("-topmost", 1)
        self.title("pgAdmin4")
        if platform == "win32":
            self.iconbitmap(resource_path(path.join("assets", "logo.jpg")))
    ...
    def systemTray(self):
        ...
        image = Image.open(resource_path(path.join("assets", "logo.jpg")))
        menu = (
            MenuItem('Show', action=self.deiconify, default=True),
        )
        self.icon = Icon("name", image, "pgAdmin4", menu)
        self.icon.run()
